...
class MVX_Product {
    ...
    function output_vendor_tab() {
        ...
        if (!$current_user_is_vendor) {
            $html .= '<select name="' . esc_attr('choose_vendor') . '" data-placeholder="'. esc_attr__("Choose vendor", "multivendorx").'" id="' . esc_attr('choose_vendor_ajax') . '" class="mvx_select_vendor" style="width:300px;" >' . $option . '</select>';
