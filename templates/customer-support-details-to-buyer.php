	...
	<tbody>
		...
		if( get_mvx_vendor_settings('is_customer_support_details', 'settings_general') ) {
			foreach ( $vendor_array as $vendor_id => $products) { 
				...
				<?php if($cc == 0) { ?>
					<tr>
					<th style="padding:10px 10px; background:none; border-right: 1px solid #e4e4e4; border-bottom: 1px solid #e4e4e4; width:100%;" align="left" valign="top"><?php esc_html_e('Vendor Details', 'multivendorx'); ?></th>
				...
				<tr>
					<td style="padding:10px 10px; background:none; border-right: 1px solid #e4e4e4; border-bottom: 1px solid #e4e4e4; width:100%;" align="left" valign="top">
						<p><strong><?php esc_html_e('Vendor Name', 'multivendorx');?> </strong><br>
						<?php echo $vendor_meta['nickname'][0] ?> </p>
						<p><strong><?php esc_html_e('Product Name', 'multivendorx');?> </strong> <br>
							 <?php echo $products; ?>
						</p>						
						<p style="border-bottom:1px solid #eeeeee; padding-bottom:10px"> <strong><?php esc_html_e('Customer Support Details', 'multivendorx');?></strong></p>
						<?php if(isset($vendor_meta['_vendor_customer_email'][0])) { ?>
						<p><strong><?php esc_html_e('Email : ','multivendorx');?></strong><br>
						...
						<p><strong><?php esc_html_e('Phone : ','multivendorx'); ?></strong> <br>
						<?php echo $vendor_meta['_vendor_customer_phone'][0]; ?></p>
						<?php }?>
						<p><strong><?php esc_html_e('Return Address ', 'multivendorx');?></strong></p>
						<p>
			...
			?>
			<tr>
					<th style="padding:10px 10px; background:none; border-right: 1px solid #e4e4e4; border-bottom: 1px solid #e4e4e4; width:100%;" align="left" valign="top"><?php esc_html_e('Customer Support Details & Product Return Address', 'multivendorx'); ?></th>
